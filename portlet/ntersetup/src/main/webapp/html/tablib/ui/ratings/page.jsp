...
String scoreString = numberFormat.format(ratingsStats.getAverageScore());
String[] scoreVars = {scoreString, "5"};
String scoreStringPartial = LanguageUtil.format(pageContext, "star-rating", scoreString);
String scoreStringTotal = LanguageUtil.format(pageContext, "the-average-rating-is-x-stars-out-of-x", scoreVars);
String label = "";
%>
					...
					<c:when test='<%= themeDisplay.isSignedIn() %>'>
						<div class="liferay-rating-vote" id="<%= randomNamespace %>ratingStar">
							<div id="<%= randomNamespace %>ratingStarContent">
								<div class="aui-rating-label-element" aria-live="polite" aria-relevant="additions">
									<liferay-ui:message key="your-rating" />
									<span class="aui-helper-hidden-accessible"><%= LanguageUtil.format(pageContext, "you-have-rated-this-x-stars-out-of-x", new Object[] {yourScore, numberOfStars}) %></span>
								</div>
								...
								%>
									...
									<label class="aui-rating-element <%= (i <= yourScore) ? "aui-rating-element-on" : StringPool.BLANK %>">
										<%
										if (i == 1) {
											if ((int) yourScore == i) label = LanguageUtil.format(pageContext, "you-have-rated-this-x-stars-out-of-x-singular", new Object[] {i, numberOfStars});
											else label = LanguageUtil.format(pageContext, "rate-this-x-stars-out-of-x-singular", new Object[] {i, numberOfStars});
										} else {
											if ((int) yourScore == i) label = LanguageUtil.format(pageContext, "you-have-rated-this-x-stars-out-of-x", new Object[] {i, numberOfStars});
											else label = LanguageUtil.format(pageContext, "rate-this-x-stars-out-of-x", new Object[] {i, numberOfStars});
										}
										%>
										<%= label %>
										<input <%= i == (int) yourScore ? "checked=\"checked\"": StringPool.BLANK %> name="rating" type="radio" value="<%= i %>" />
									</label>
				...
				<div class="liferay-rating-score" id="<%= randomNamespace %>ratingScore">
					<div id="<%= randomNamespace %>ratingScoreContent">
						<div class="aui-rating-label-element">
							...
							<span class="aui-helper-hidden-accessible"><%= scoreStringTotal %></span>
						</div>
						...
						%>
							...
							<div class="aui-rating-element <%= (i <= ratingsStats.getAverageScore()) ? "aui-rating-element-on" : StringPool.BLANK %>"></div>
					...
					<c:when test='<%= themeDisplay.isSignedIn() %>'>
						<div class="aui-thumbrating liferay-rating-vote" id="<%= randomNamespace %>ratingThumb">
							<div class="aui-helper-clearfix aui-rating-content aui-thumbrating-content" id="<%= randomNamespace %>ratingThumbContent">
								<div class="aui-rating-label-element" aria-live="polite" aria-relevant="text">
									<c:choose>
								...
								<label class="aui-rating-element aui-rating-element-<%= (yourScore > 0) ? "on" : "off" %> aui-rating-thumb-up">
									<%= (yourScore == 1) ? LanguageUtil.get(pageContext, "you-have-rated-this-as-good") : LanguageUtil.get(pageContext, "rate-this-as-good") %>
									<input name="ratingThumb" type="radio" value="up" />
								</label>

								<label class="aui-rating-element aui-rating-element-<%= (yourScore < 0) ? "on" : "off" %> aui-rating-thumb-down">
									<%= (yourScore == -1) ? LanguageUtil.get(pageContext, "you-have-rated-this-as-bad") : LanguageUtil.get(pageContext, "rate-this-as-bad") %>
									<input name="ratingThumb" type="radio" value="down" />
								</label>
							</div>
						</div>
					</c:when>
						...
						<div class="aui-thumbrating liferay-rating-vote" id="<%= randomNamespace %>ratingThumb">
							<div class="aui-helper-clearfix aui-rating-content aui-thumbrating-content" id="<%= randomNamespace %>ratingThumbContent">
								<div class="aui-rating-label-element" aria-live="polite" aria-relevant="text">
									...
									<a href="<%= themeDisplay.getURLSignIn() %>" class="aui-rating-sign-in"><liferay-ui:message key="sign-in-to-vote" /></a>
								</div>
							</div>
						</div>
					</c:otherwise>
