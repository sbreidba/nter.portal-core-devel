...
/**
 * The base implementation of the external link local service.
 *
 * <p>
 * This implementation exists only as a container for the default service methods generated by ServiceBuilder. All custom service methods should be put in {@link org.nterlearning.datamodel.catalog.service.impl.ExternalLinkLocalServiceImpl}.
 * </p>
 *
...
public abstract class ExternalLinkLocalServiceBaseImpl
    implements ExternalLinkLocalService, IdentifiableBean {
    private static Log _log = LogFactoryUtil.getLog(ExternalLinkLocalServiceBaseImpl.class);
     ...
     * NOTE FOR DEVELOPERS:
     *
     * Never modify or reference this class directly. Always use {@link org.nterlearning.datamodel.catalog.service.ExternalLinkLocalServiceUtil} to access the external link local service.
     */
    ...
    public ExternalLink addExternalLink(ExternalLink externalLink)
        ...
        externalLink = externalLinkPersistence.update(externalLink, false);

        Indexer indexer = IndexerRegistryUtil.getIndexer(getModelClassName());

        if (indexer != null) {
            try {
                indexer.reindex(externalLink);
     ...
     */
    public ExternalLink createExternalLink(long linkId) {
        return externalLinkPersistence.create(linkId);
    }
    ...
    public void deleteExternalLink(long linkId)
        ...
        ExternalLink externalLink = externalLinkPersistence.remove(linkId);

        Indexer indexer = IndexerRegistryUtil.getIndexer(getModelClassName());

        if (indexer != null) {
            try {
                indexer.delete(externalLink);
            } catch (SearchException se) {
                if (_log.isWarnEnabled()) {
                    _log.warn(se, se);
                }
    ...
    public void deleteExternalLink(ExternalLink externalLink)
        throws SystemException {
        externalLinkPersistence.remove(externalLink);
    ...
    public ExternalLink fetchExternalLink(long linkId)
        throws SystemException {
        return externalLinkPersistence.fetchByPrimaryKey(linkId);
    }
     ...
     * @throws SystemException if a system exception occurred
     */
    public ExternalLink getExternalLink(long linkId)
        throws PortalException, SystemException {
        return externalLinkPersistence.findByPrimaryKey(linkId);
    }
     ...
     * @throws SystemException if a system exception occurred
     */
    public int getExternalLinksCount() throws SystemException {
    ...
    public ExternalLink updateExternalLink(ExternalLink externalLink)
        throws SystemException {
        return updateExternalLink(externalLink, true);
    }
    ...
    public ExternalLink updateExternalLink(ExternalLink externalLink,
        ...
        externalLink = externalLinkPersistence.update(externalLink, merge);
    ...
    public void afterPropertiesSet() {
        PersistedModelLocalServiceRegistryUtil.register("org.nterlearning.datamodel.catalog.model.ExternalLink",
            externalLinkLocalService);
    }

    public void destroy() {
        PersistedModelLocalServiceRegistryUtil.unregister(
            "org.nterlearning.datamodel.catalog.model.ExternalLink");
    }
